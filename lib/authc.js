...
var clientAuth = function(req, res, next) {
    ...
    var apiCall = req.header('Ilimi-Api-Call');
    if(apiCall == "true") {
        Step(
            function() {
                Client.get(req.header('Consumer-Key'), this);
            },
            function(err, client) {
                if (err) throw err;
...
var userAuth = function(req, res, next) {
    ...
    var apiCall = req.header('Ilimi-Api-Call');
    if(apiCall == "true") {
        getUser(req.header('User-Name'), req.header('Consumer-Key'), function(error, deetz) {
            console.log('API authentication');
            log.debug(deetz, "Authentication details");

            if (!deetz || !deetz.user || !deetz.client) {
                log.debug("Incorrect auth details.");
...
getUser = function(username, client_id, callback) {
    var user, client;
    Step(
        function() {
            Client.get(client_id, this);
        },
        function(err, res) {
            if (err) throw err;
            client = res;
        ...
        function(err, res) {
            if (err) {
                ...
                user = res;
                callback(null, {id: username, user: user, client: client});
            }
        }
    );
};
